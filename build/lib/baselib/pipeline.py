...
from core.typecheck import type_check
...
def get_subfolders(path:str) -> list[tuple[str,str]]:
    ...
    """
    import os
    return [(subfolder,os.path.join(path,subfolder)) for subfolder in os.listdir(path) if os.path.isdir(os.path.join(path,subfolder))]