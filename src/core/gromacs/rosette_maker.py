...
def make_rosette[T:mole.IMolecule](monomer: T, n: int, size: float, angle: float, degree: bool = True) -> mole.Substructure[T]:
    """
    ...
    """
    rosette = mole.Substructure([copy.deepcopy(monomer) for i in range(n)])


    radius = size/2
    # move monomer to circle
    for i in range(n):
        rosette.get_children()[i].rotate(Rotation.from_euler('y', -angle, degrees=degree))
        # move head atom to circle
        rosette.get_children()[i].translate(np.array([radius, 0, 0]))
        # rotate monomer in a radial direction
        rosette.get_children()[i].rotate(Rotation.from_euler('y', 360/n*i, degrees=True))
    return rosette
    #for debug
    # rosette.extract_xyz('')

def make_rosette2[T:mole.IMolecule](monomer: T, n: int, size: float) -> mole.Substructure[T]:
    """
    make rosette from monomer
    n: number of monomer
    size: diamiter of circle setting monomer top atom
    """
    rosette:mole.Substructure[T] = mole.Substructure([copy.deepcopy(monomer) for i in range(n)])
...
def make_half_rosette2[T:mole.IMolecule](monomer: T, n: int, size: float) -> mole.Substructure[T]:
    """
    make rosette from monomer
...
def make_oligorosette[T:mole.IMolecule](rosette:T, n:int, length:float, angle:float, slip:float=0, degree:bool=True) -> mole.Substructure[T]:
    """
    ...
    """
    oligorosette = mole.Substructure[T]([copy.deepcopy(rosette) for i in range(n)])
    ...
    for i,rosette in enumerate(oligorosette.get_children()):
        ...
        rosette.translate(np.array([radius*np.sin(i* angle * np.pi/180), radius*np.cos(i* angle * np.pi/180), 0]))
