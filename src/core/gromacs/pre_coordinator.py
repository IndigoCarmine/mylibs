...
def pre_coordinate[T:i.IMolecule](molecule: T,topO:int,aromaticsideO:int,aromaticothersideO:int) -> T:
    """
    ...
    vector = molecule.get_child(aromaticsideO).coordinate + \
    ...
    rot = Rotation.from_euler(
        "x", np.arctan2(vector[1],vector[2]), degrees=False)
    molecule.rotate(rot)
...
def precooredinate2[T:i.IMolecule](molecule: T,topO:int,aromaticsideNH:int,aromaticothersideO:int) -> T:
    """
    pre-coordinate molecule
    """
    ...
    rot = Rotation.from_euler(
        "y", np.arctan2(vector[2],vector[0]), degrees=False) 
    molecule.rotate(rot)
    ...
    rot = Rotation.from_euler(
        "x", np.arctan2(vector[2],-vector[1]), degrees=False)
