...
class XyzMolecule(IMolecule):
    ...
    def make_from(cls, mol: IMolecule, name: str, index: int) -> "XyzMolecule":
        return cls(name,index,[XyzAtom(atom.symbol,atom.index,atom.coordinate) for atom in mol.get_children()])
    ...
    def make(cls, atoms: list[AtomBase]):
        return cls("molecule", 0, [XyzAtom(atom.symbol,atom.index,atom.coordinate) for atom in atoms])
    ...
    def from_mol2_file(cls, path: str):
        ...
        for line in lines:
            ...
            atom_list.append( XyzAtom(line_split[1], int(line_split[0]), np.array(line_split[2:5], dtype=float))) 
    ...
    def save_xyz(self, filename: str|None = None):
        ...
        with open(str(filename), 'x')as f:
            ...
            for atom in mol.children:
                f.write(atom.symbol+' '+' '.join([format_float(s)
                        for s in atom.coordinate]) + '\n')
    ...
    def generate_gaussian_input(self,dir_path:str,gaussan_pram:str | None=None)->None:
        ...
        f.write('0 1\n')
        for atom in self.children:
            f.write(atom.symbol + ' ' + ' '.join([format_float(c) for c in atom.coordinate]) + '\n')
        f.write('\n')
        f.write('\n')
        f.close()
...
class XyzSubstructure(Substructure):
    ...
    def from_Substructure(cls,sub:Substructure):
        ...
        return cls([XyzMolecule.make_from(mol, mol.name, mol.index) for mol in children],"")
    ...
    def extract_xyz(self,filename:str)->None:
        ...
        f.write(str(sum([molecule.sizeofAtoms() for molecule in temp_agg.molecules])) + '\n')
        f.write('\n')
    ...
    def generate_gaussian_input(self,dir_path:str,gaussan_pram:str|None=None,fragment=False)->None:
        """
       ...
       # set molcule name as fragment index
        ...
        if fragment:
            ...
            for molecule in temp_agg.molecules:
                for atom in molecule.children:
                    f.write(atom.symbol + '(Fragment='+ molecule.name + ') ' + ' '.join([str(c) for c in atom.coordinate]) + '\n')
        else:
            # write charge and spin multiplicity
            f.write('0 1\n')
            #write number of atoms
