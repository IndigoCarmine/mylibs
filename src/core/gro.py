...
class GroFile(IMolecule[GroAtom]):
    ...
    def generate_xyz_text(self) -> list[str]:
        ...
        for atom in self.atoms:
            xyz.append(f"{atom.atom_symbol} {format_float(atom.coordinate[0] * NM_TO_ANGSTROM)} {format_float(atom.coordinate[1] * NM_TO_ANGSTROM)} {format_float(atom.coordinate[2] * NM_TO_ANGSTROM)}")
        return xyz
